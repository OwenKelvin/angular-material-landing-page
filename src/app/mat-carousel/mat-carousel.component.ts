...
export class MatCarouselComponent
  ...
  @Input() public svgIconOverrides: SvgIconOverrides | null = null;
  ...
  @Output()
  public slideChange: EventEmitter<number> = new EventEmitter<number>();
  ...
  public ngAfterViewInit(): void {
    ...
    this.slides$
      ...
      .subscribe(value => this.resetSlides(Number(value)));
  }
  ...
  private isOutOfBounds(): boolean {
    ...
    const left =
      sign *
      ((this.carouselList as ElementRef).nativeElement.getBoundingClientRect().left -
        Number((this.carouselList as ElementRef).nativeElement.offsetParent?.getBoundingClientRect()
          .left));
    const lastIndex = this.slidesList.length - 1;
    const width = -this.getWidth() * lastIndex;

    return (
      ((this.listKeyManager as any).activeItemIndex === 0 && left >= 0) ||
      ((this.listKeyManager as any).activeItemIndex === lastIndex && left <= width)
    );
  }
  ...
  private getOffset(): number {
    const offset = Number(this.listKeyManager!.activeItemIndex) * this.getWidth();
    const sign = this.orientation === 'rtl' ? 1 : -1;
    return sign * offset;
  }
